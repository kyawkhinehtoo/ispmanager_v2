      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="inline-flex w-full divide-y divide-gray-200">
          <ul id="tabs" class="flex">
            <li class="px-2 lg:px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider"
            ...
            <li class="px-2 lg:px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase tracking-wider"
              :class="[tab == 2 ? 'border-b-2 border-indigo-400 -mb-px' : 'opacity-50']"><a href="#"
                @click="tabClick(2)" preserve-state>Details</a></li>
          </ul>
        </div>
        ...
        <div v-show="tab == 2">
              ...
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col"
                    ...
                    DN/Port</th>
                  <th scope="col"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">SN
                    Name</th>
                  <th scope="col"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    Total Ports</th>
          ...
          <span v-if="sns.total" class="w-full block mt-4">
            <label class="text-xs text-gray-600">{{ sns.data.length }} SN Port List in Current Page. Total Number of SN
              Ports : {{ sns.total }}</label>
          </span>
          <span v-if="sns.links">
        ...
        <div v-show="tab == 1">
              ...
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col"
                    ...
                    Name</th>
                  <th scope="col"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DN
                    Name</th>
                  <th scope="col"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    Total Ports</th>
          ...
          <span v-if="overall.total" class="w-full block mt-4">
            <label class="text-xs text-gray-600">{{ overall.data.length }} SN List in Current Page. Total Number of SNs
              : {{
                overall.total }}</label>
          </span>
      ...
      <div>
        <div class="mt-4 text-sm">
          ...
          <div class="mt-1 flex rounded-md shadow-sm" v-if="overall.length !== 0 && tab == 2">
            <multiselect deselect-label="Selected already" :options="overall" track-by="id" label="name"
              v-model="form.sn" :allow-empty="true"></multiselect>
          </div>
          <jet-input-error :message="form.error" class="mt-2" />
          <jet-input type="number" class="mt-1 block w-full" placeholder="SN Port Total" ref="number"
            v-model="form.port" />

          <jet-input-error :message="form.error" class="mt-2" />
          <jet-input type="text" class="mt-1 block w-full" placeholder="SN Description" ref="text"
  ...
  setup(props) {
    ...
    function deleteNode() {
      ...
      if (tab.value == 2) {
        ...
        router.post("/snport/group/" + data.id, data, {
          preserveScroll: true,
          preserveState: true,
          onSuccess: (page) => {
            id.value = false;
